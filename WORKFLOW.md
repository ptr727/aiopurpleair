...
- **Shipped path** - the paths-filter inclusion list on the publisher's push trigger: `src/aiopurpleair/**`, `pyproject.toml`, `version.json`, `uv.lock`. Touching one of these on an **allowlisted bot** merge to `main` publishes, and a docs, test, or workflow-only merge does not. The filter decides whether the workflow runs, and the `plan` job then decides whether it publishes.
