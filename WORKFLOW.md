...
**Validation.** Proving **D8.5** takes three checks, not one, and [`AUDIT.md`](./AUDIT.md) section 4 runs all three. The hub-hosted `repo-config/configure.sh check` covers the rulesets, the general settings, the security features, and the labels, against payloads the hub carries. `spec/audit.py`, from the same hub checkout, asserts the secret **names**, since values cannot be read back. The `pypi` deployment environment is checked **by hand**, because no script covers it, and its intended state is recorded as prose in [`AUDIT.md`](./AUDIT.md) section 4 rather than as a payload this repository carries. This repository keeps no repository-configuration payloads at all. The App installation and the PyPI publisher registration stay manual/best-effort items.
