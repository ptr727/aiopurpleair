...
- **Names.** Reusable workflow `name:` ends in **"task"**, entry names in **"action"**. Every job `name:` ends in **"job"**, every step `name:` in **"step"** - the aggregator included (`Check pull request workflow status job`). A job name bound as a ruleset required-check `context:` is changed only **in lockstep** with the live ruleset, which is applied from the hub's payloads rather than from the record in [`repo-config/`](./repo-config/).
...
- **D8.5 Configuration outside the YAML is present.** Output: the secrets (`CODEGEN_APP_CLIENT_ID`, `CODEGEN_APP_PRIVATE_KEY`, and `CODECOV_TOKEN`, each in both the Actions and Dependabot stores), the `main` and `develop` rulesets, the repository settings, and the PyPI Trusted Publisher registration with its `pypi` environment restricted to `main` and `develop` are all in place. *Prevents: a green-looking repo whose first real publish or auto-merge fails on a missing secret, an unenforced ruleset, or an unregistered publisher.* Detail and the audit procedure are in section 6. **The `pypi` environment half of this guarantee is checked by hand.** It is still part of the audit, and [`AUDIT.md`](./AUDIT.md) section 4 carries the commands and the expected values; what changed is that no script performs it. The carried script that asserted its custom-branch-policy mode and its exact allowed-branch set was retired when the repository-configuration script became hub-hosted, and the hub's script manages no deployment environment, so nothing mechanical now notices that policy being widened, deleted, or flipped to protected-branches mode.
...
| S8 | PR with a ruff, mypy, pyright, pytest, markdownlint, cspell, EditorConfig, or actionlint failure | the owning `validate-task` job reds, so `validate` reds and the aggregator blocks the merge | D1.2, D1.5 |
...
**Validation.** Proving **D8.5** takes three checks, not one, and [`AUDIT.md`](./AUDIT.md) section 4 runs all three. The hub-hosted `repo-config/configure.sh check` covers the rulesets, the general settings, the security features, and the labels, against payloads the hub carries. `spec/audit.py`, from the same hub checkout, asserts the secret **names**, since values cannot be read back. The `pypi` deployment environment is checked **by hand**, because no script covers it. [`repo-config/`](./repo-config/) here is a record, not an input: `environment-pypi.json` is what the by-hand environment check compares against, and the two `ruleset-*.json` files are compared by nothing at all, so they can drift silently and are reconciled by hand when the rulesets change. The App installation and the PyPI publisher registration stay manual/best-effort items.
