...
- **Recorded N/A** (single target, no Docker **image**, no wrapper, no deploy): **D1.1** and **D1.4** (a paths-filter over multiple targets, this repo smoke-builds its one target on every pull request), **D3.5** (no upstream-version tracker), **D4.6** (no deploy to an owned host), **D5.2**, **D5.3**, **D5.5** and **D5.6** (there is no delete step here for D5.2 to gate or D5.3 to make best-effort, no cleanup broad enough for D5.5, and no durable destination beyond PyPI, whose versions are immutable), **D6.3** and **D6.4** (one target, so no branch-suffix collision surface and no add/drop matrix), **D7.4** (no optional-dependency chaining), **D8.3** (no tracker), and **D9.4** (no Docker layer cache).
...
**Self-testing is now partial, and deliberately so.** A pull request still head-resolves the reusable workflows it calls by local path, so a change to [`build-release-task.yml`](./.github/workflows/build-release-task.yml) or [`get-version-task.yml`](./.github/workflows/get-version-task.yml) is exercised by the pull request that makes it. The validate gate is **not** self-tested any more, because it is the hub's and is reached at a pin: a change to it lands in the hub, and this repository sees it when the pin moves. That is the trade the `retire` disposition asks for, and it is why the pin appears in two callers that must carry the same SHA.
...
**The smoke build now versions an ephemeral ref.** On a `pull_request` event [`get-version-task.yml`](./.github/workflows/get-version-task.yml) checks out `refs/pull/N/merge` and exports that commit as `GitCommitId`, and the `build` job then re-fetches it by bare SHA. GitHub recomputes the merge ref when the **base** branch moves, so a base push between the two jobs can leave that SHA named by no ref and fail the checkout. `cancel-in-progress` covers a push to the pull request's own head and not this. Under the old `push` trigger the SHA was always a durable branch commit. The exposure is a smoke build failing spuriously and rerunning green rather than anything shipping wrong, and it is expected to go away with the release-chain adoption, which replaces this task with the hub's.
...
**Pull request (CI), `test-pull-request.yml`.** Every pull request, whatever its base branch, runs the hub's validate task at a pin (lint + the 3.13/3.14 pytest matrix + the pyright hook) and a head-resolved smoke build, and a single aggregator produces the ruleset-bound required check (D1, D5).

```mermaid
flowchart TD
    T(["pull_request: any base branch<br/>(or workflow_dispatch)"]):::trig
    T --> V["validate job<br/>(hub validate-task.yml @pin)"]
    T --> SB["smoke-build job<br/>(build-release-task.yml, smoke: true)<br/>build wheel + sdist, no publish"]
...
- **D6.1 CI and the publisher are separate, and each builds one branch.** Output: `test-pull-request` and `publish-release` are separate workflows with separate concurrency, so they never race. CI validates the pull request's merge ref and publishes nothing, handing its smoke build the **base** branch as `branch:`, and the publisher versions, builds, and tags exactly its own trigger ref. The `build` to `publish-pypi` handoff is by artifact `name:`, not `artifact-ids:`. *Prevents: cross-branch ref mixing, and a CI run racing a publish on the same ref.*
- **D6.2 Branch drives config.** **Recorded drift.** `build-release-task` reads `inputs.branch` for the artifact name and the PEP 440 `.dev0` derivation, but reads `github.ref_name` for `validate-release`'s `REF_NAME` and for the release's `prerelease` expression. The publisher's `publish` job passes `branch: ${{ github.ref_name }}`, but CI's `smoke-build` now passes `branch: ${{ github.base_ref || github.ref_name }}`, because on a `pull_request` event `github.ref_name` is the pull request ref such as `123/merge` rather than a branch. So on every pull request `inputs.branch` reads `develop` or `main` while the `github.ref_name` reads inside the task read `123/merge`. **The two reads no longer agree on a pull request, and this drift item is now live rather than hypothetical.** It is harmless only because the smoke path skips both consumers: `validate-release` is skipped on `smoke: true`, and the artifact upload is skipped too, so neither the version gate nor an artifact name is computed from the mismatched pair. On the publisher, where those consumers do run, the trigger is a push to `main` or a dispatch, so `github.ref_name` is a real branch and both reads agree as before. What would make it bite is a smoke build that started uploading, or a version gate that stopped skipping on smoke, so treat either change as reopening this item. *Prevents, where satisfied: a task behaving differently depending on how it was reached.*
...
| S3 | PR changing only `.github/workflows/**` | a changed **local** task (`build-release-task`, `get-version-task`) is exercised head-resolved, a changed validate pin is not, since that task is the hub's, aggregator success | D1.2, D1.5 |
