...
- **D2.2 Release branch matches version classification.** Input: a real (non-smoke) release build. Output: the gate fails loudly if `main` carries a prerelease `-` segment, or a non-`main` branch carries none. On a smoke build the **check exits early while the job still reports success**. The reason is not a detached head, since no workflow here carries a `pull_request` trigger and smoke runs on `push`: it is that a feature branch always classifies as prerelease, so the gate's `main` arm would trip on every smoke of a branch whose eventual publish target is `main`. *Prevents: a `develop` build published as stable, and the gate blocking every promotion PR.*
...
- **D3.3 Version floor + git height.** Output: [`version.json`](./version.json) sets the major.minor floor, currently `1.0`, and NBGV appends the git height as the patch, never bumped on a cadence. It also sets **`versionHeightOffset: -1`**, so the published patch is the height minus one. That offset is load-bearing rather than incidental: removing it shifts every subsequent release by one, and a PyPI version cannot be re-pushed, so the shift would collide with an already-published version. NBGV and `version.json` are retained although this repo compiles nothing, because they own the tag. *(Who raises the floor and when is a human-process rule in `GOVERNANCE.md` "Release Model", with this repo's own facts in `OPERATIONS.md` "Release and Versioning Facts".)*
...
- **D6.2 Branch drives config.** **Recorded drift.** `build-release-task` reads `inputs.branch` for the artifact name and the PEP 440 `.dev0` derivation, but reads `github.ref_name` for `validate-release`'s `REF_NAME` and for the release's `prerelease` expression. Today the only caller passes `branch: ${{ github.ref_name }}`, so the two agree and nothing misbehaves. They would diverge the moment a caller passed a different branch: the version gate and the release flag would follow the run's ref while the wheel suffix and the artifact name followed the input, cutting a stable-marked release for a `.dev0` wheel and then failing to download an artifact named for the other branch. *Prevents, where satisfied: a task behaving differently depending on how it was reached.*
