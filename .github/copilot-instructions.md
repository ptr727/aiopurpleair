...
{
  repository(owner: "ptr727", name: "aiopurpleair") {
...
gh api repos/ptr727/aiopurpleair/issues/<N>/comments --jq \
...
Read [AGENTS.md](../AGENTS.md) for the entry point and its map, [GOVERNANCE.md](../GOVERNANCE.md) for the cross-cutting rules, and [OPERATIONS.md](../OPERATIONS.md) for this repo's own conventions. For code-style rules, [`CODESTYLE.md`](../CODESTYLE.md) (its General section plus the relevant language section) is authoritative. Don't restate any of these files' rules in commit bodies or PR descriptions, and keep those focused on the change itself.

**In a derived repo:** if you find a discrepancy that should be fixed in the template itself (this file or GOVERNANCE.md is out of date, a rule is missing, something bit this repo and would bite the next), open an issue upstream in [`ptr727/ProjectTemplate`](https://github.com/ptr727/ProjectTemplate) rather than only fixing it locally.
