...
All 11 paths of the published API spec are implemented, and every response shape is verified against the live API: keys, sensors (list, single, history JSON and CSV), organization, and the full Groups API (groups CRUD, member add and remove, members data, single member, member history CSV).
