...
- **Coverage**: all 11 spec paths (API `1.2.0`) are implemented - keys, sensors (list, single, history JSON/CSV), organization, and the full Groups API. The Groups member-*data* read shapes (`GET /groups/:id/members` and the single-member variant) are modeled after the sensors responses per the docs (the members-management write account owned no addable sensor to live-capture them); the groups list/detail/create and both history CSVs are live-verified.
