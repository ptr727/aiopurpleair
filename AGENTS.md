...
- **A pre-commit hook runs ruff** (`uv run ruff format` + `uv run ruff check --fix`) on staged Python files. Install it with `uvx prek install` (prek, a fast drop-in for pre-commit). It is language formatting only - the doc linters and full checks run in CI and the VS Code Lint tasks.
