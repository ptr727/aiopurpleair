...
2. **Spelling**: US English throughout, in code comments, identifiers, commit messages, PR descriptions, and documentation alike. Prefer the American form of every word that has two: *behavior*, *color*, *recognize*, *organize*, and *canceled* with one L, never their British spellings. **This governs prose, not identifiers.** A fixed name defined elsewhere is spelled the way its definition spells it, so GitHub Actions' `cancelled()` status function and the `cancelled` job result keep their two Ls, as does any API field, enum member, or config key that arrives spelled that way. Project-specific terms (`aiopurpleair`, `PurpleAir`, `aiohttp`, `pydantic`, `syrupy`, `NBGV`) are correct, not misspellings.
