...
- **This repository carries no repository-configuration payloads.** The settings, labels, and branch rulesets that [`GOVERNANCE.md`](./GOVERNANCE.md) "Branching Model" describes are the hub's, applied and checked by the hub-hosted `repo-config/configure.sh` against the hub's own payloads. Running `repo-config/configure.sh check ptr727/aiopurpleair release` from a hub checkout at `main` is the whole of it, so there is no local copy to keep in step and none to drift. Do not restate the ruleset details here or reintroduce a payload directory.
- **The `pypi` deployment environment is the exception, and it is unmanaged.** The hub's script handles no deployment environment at all, so nothing applies or checks this one. Its intended state is recorded as prose in [`AUDIT.md`](./AUDIT.md) section 4, with the `gh api` commands and the exact expected values, and it is configured and verified by hand.
