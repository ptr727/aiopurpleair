...
- [`version.json`](./version.json) holds the SemVer base `1.0` (major.minor), the `publicReleaseRefSpec` regex matching `^refs/heads/main$`, and `versionHeightOffset: -1`. NBGV takes the git commit height plus that offset as the patch component, so the patch is the height minus one, and on any ref not matching `publicReleaseRefSpec` appends a `-g{sha}` prerelease segment. So `main` produces clean SemVer such as `1.0.5`, and `develop` produces prereleases such as `1.0.5-g1a2b3c4`.
