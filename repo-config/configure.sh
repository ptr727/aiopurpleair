...
# Idempotent: `apply` configures a repo to match the JSON in this directory; `check` validates an existing
# repo and exits non-zero on drift (the 5D audit). Run from anywhere; the target repo is resolved from the
# current `gh` context unless $REPO is set (owner/name).
#
...
# Secrets by store (names only; values are never readable via the API). The App credentials must be set in
# BOTH stores: the merge-bot reads them from Actions on a maintainer-triggered run, but from the Dependabot
# store when it acts on a Dependabot PR, because Dependabot-triggered runs are given the Dependabot secret
# store, not Actions secrets. CODECOV_TOKEN is Actions-only (coverage upload is not a Dependabot run). PyPI
# publishing uses OIDC Trusted Publishing, so there is no stored publish API key.
